...
//	std::cout << x << std::endl;
	fstream file0, file1;
    string s1, s2, s3;
	file0.open("0.kti", std::ios::binary | std::ios::in | std::ios::out);
	file1.open("1.kti", std::ios::binary | std::ios::in | std::ios::out);
    int i, j;
...
{
    fstream file;
	file.open("0.kti", std::ios::binary | std::ios::in | std::ios::out);
	if(!file.good())
		init_total();
	else
	{
